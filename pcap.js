...
tcp_tracker.on("session", function (session) {
    ...
    session.on("data send", function (session, data) {
       // console.log(session.src_name + " -> " + session.dst_name + "发送 " + session.send_bytes_payload + " + " + data.length + " bytes");
    });
    session.on("data recv", function (session, data) {
        console.log(session.dst_name + " -> " + session.src_name + "接收" + session.recv_bytes_payload + " + " + data.length + " bytes");
        });
